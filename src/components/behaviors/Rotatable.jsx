...
const Rotatable = forwardRef(({ children, updateHandler }, rotatableRef) => {
  ...
  const start = useCallback(
    (mousePostionX, mousePostionY) => {
      console.log(
        'Rotatable start',
        parameterRef.current.originAngle,
        parameterRef.current.startAngle
      );
  ...
  const end = useCallback(() => {
    ...
    if (parameterRef.current.isDragging) {
      ...
      if (angleMatch && angleMatch[1]) {
        parameterRef.current.originAngle +=
          parseFloat(angleMatch[1]) - parameterRef.current.originAngle;
      }
  ...
  return (
    <div
      data-testid="rotatable-behavior"
      style={rotatableStyles}
      ref={rotatableRef}
      className="rotatable-behavior">
      <RotatableCS ref={controlRef} rotation={rotation} />
      {children}
    </div>
  );
});
