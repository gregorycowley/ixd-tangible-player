...
/**
 * Above is assumed to be boilerplate code that is not relevant to the
 * code below.
 */
...
class TEConnect extends events_1.EventEmitter {
   ...
   * @param {object} client - An optional net.Socket client to use for the connection.
   * Touch events will be attached to the Window by default.
   * @memberof TEConnect
   */
  constructor(port = 4949, host = '127.0.0.1', selector, alternateClient) {
    super();
    this.scaleFunction = (x, y) => {
      return { x: x, y: y };
    };
    if (alternateClient !== undefined) {
      this._client = alternateClient.connect({ host: host, port });
    } else {
      this._client = net.connect({ host: host, port });
    }
